...
public class coutomerdbutil {
	...
	public static List<Customer> validate(String division,String password){
		...
		try {
			...
			String sql="select * from division where name='"+division+"'and password='"+password+"'";
	...
	int convertedID = Integer.parseInt(id);
	 ...
	 int submit = 0;
    	...
    	try {
    		...
    		String sql_ = "select * from division where id='"+convertedID+"'";
    		...
    		if(submit==0) {
    		String sql = "update division set name='"+division+"',distric='"+distric+"',submit='"+1+"',party1='"+party1+"',party2='"+party2+"',party3='"+party3+"'"
    				+ "where id='"+id+"'";
...
public static List<Customer> getDetails(String Id) {
	...
	try {
		...
		con = DBConnect.getConnection();
		stmt = con.createStatement();
		String sql = "select * from division where id='"+convertedID+"'";
